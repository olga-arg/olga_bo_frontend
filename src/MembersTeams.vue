  ...
  <div className="bg-[#F4F4F4]">
    ...
    <div class="p-4 sm:ml-64 z-0">
      <div class="px-24">
          ...
          <div v-if="showAddMember" className="w-96 p-10 bg-[#1C2E3D] rounded-2xl">
            <div v-if="showSendInvite">
              <div className="flex justify-between">
                <p className="text-[#DE848B] text-xl font-medium">Crea un Equipo</p>
              ...
              <div className="flex flex-col gap-2 my-10">
                <input v-model="name" type="name" name="name" id="name" className="w-full h-14 rounded-md p-4 text-sm bg-[#F4F4F4]" placeholder="Nombre del equipo" required />
                <input
                  v-model="annualBudget"
                  type="number"
                  name="annualBudget"
                  id="annualBudget"
                  className="w-full h-14 rounded-md p-4 text-sm bg-[#F4F4F4]"
                  placeholder="Presupuesto anual estimado"
                  required
                />
              </div>
            </div>
        ...
        <Suspense>
          <template #default>
            <MembersTeamsView :key="forceReload" :filters="sendFilters"></MembersTeamsView>
          </template>
          <template #fallback>
...
export default {
  ...
  methods: {
    ...
    async sendInvite() {
      if (this.name === undefined || this.annualBudget === undefined) {
        return
      }
